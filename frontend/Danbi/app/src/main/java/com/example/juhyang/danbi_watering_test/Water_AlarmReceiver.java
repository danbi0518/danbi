...
public class Water_AlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (extra != null) {
            ...
            if (isOneTime) {
                ...
                Toast.makeText(context, "알람이 울려염 반복 x", Toast.LENGTH_LONG).show();
            }
                ...
                if (!week[cal.get(Calendar.DAY_OF_WEEK)])
                    return;
                Toast.makeText(context, "알람이 울려염 반복 o", Toast.LENGTH_LONG).show();
    ...
    public void watering(final Water_AlarmData alramData, final Context context) {
        ...
        String onoff = "1";
        Call<Water_model> call = api.GetData(onoff, String.valueOf(alramData.minute)); //onoff 와 분에 관한 정보를 http로 송신
        call.enqueue(new Callback<Water_model>() { //비동기방식 통신
            ...
            public void onResponse(Response<Water_model> response, Retrofit retrofit) {
                if (response.body().result.equals("1")) {
                    ...
                    timer[0] = new CountDownTimer(alramData.minute * 60 * 1000, 1000) {
